...
| D2.1 | Solveur mecanique FEniCSx (Holzapfel-Ogden + BCS) | 80% | Formulation quasi-incompressible mixte P2-P1 (Taylor-Hood) validee, p direct, garde-fou domaine corrige (bug petsc4py 3.20.0 setFunctionDomainError absent -> remplace par residu fini force). DEUX convergences completes (lam=1.0) sur maillage grossier (patient001_coarse5, 5686 tets), fibres tangentielles et LDRB. Maillage FIN (34025 tets) : bloque durablement a lam~0.008 (singularite locale, confirmee independante de kappa_vol -- teste 1e6 et 1e5, meme signature d'echec). Maillage intermediaire (19894 tets, target_mm=3.5) : meme resolution radiale insuffisante que le grossier (~3.6 elements dans l'epaisseur pariétale, cause structurelle -- spacing Z=10mm systematique sur les 10 patients ACDC, confirme). DECISION ACTEE (2026-07-12, verifiee contre heart_digital_twin_proposal.pdf) : le maillage grossier est SUFFISANT pour les deliverables du projet -- D2.1 ne specifie pas de resolution transmurale, D3.1 vise un surrogate >=100x plus rapide (le full-order sert uniquement a generer le DoE, jamais execute en temps reel), et les 2 cas d'usage clinique WP5 (stratification risque arythmie, optimisation CRT) reposent sur EP + metriques globales/segmentaires, pas sur un gradient transmural fin. Maillage grossier retenu comme reference pour le DoE. |
