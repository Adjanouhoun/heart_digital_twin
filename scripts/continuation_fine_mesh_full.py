"""
Continuation ADAPTATIVE COMPLETE sur le MAILLAGE FIN DE PRODUCTION
(patient001, ~34025 tets apres filtrage), formulation validee (p direct,
garde-fou domaine, continuation avec dlam CROISSANT jusqu'a 0.05 -- pas
le dlam fixe du test kappa d'hier soir).

Objectif : obtenir le VRAI temps de convergence complete (lam=1.0) sur
le maillage de production, pour dimensionner le DoE multi-fidelite
(nombre de runs haute-fidelite affordables). Protege par checkpoint
disque -- peut tourner sur plusieurs jours sans risque de perte.
"""
import sys, time, os
...
print(f"\n=== CONTINUATION COMPLETE MAILLAGE FIN (T_max=135kPa) ===", flush=True)
...
print(f"TEMPS TOTAL ESTIME POUR lam=1.0 (extrapolation si non converge) : "
      f"a calculer manuellement selon la progression", flush=True)
