...
mech_params = MechanicsParameters(
    ...
    checkpoint_dir=f"/cdt/doe_checkpoints_fixed/{job_id}",
)
...
os.makedirs("/cdt/doe_full_results_fixed", exist_ok=True)
np.savez(f"/cdt/doe_full_results_fixed/{job_id}.npz",
...
os.makedirs("/cdt/doe_results_fixed", exist_ok=True)
with open(f"/cdt/doe_results_fixed/{job_id}.json", "w") as f:
